...
class ConeLocator():
    ...
    def cd_color_segmentation(self, img):
        ...
        '''
        hsv = cv2.cvtColor(self.restrict_view(img), cv2.COLOR_BGR2HSV) # convert image to HSV color space
    ...
    def restrict_view(self,img):
        """Will make black bar lines on top and bottom of the video image, restricting
        the cameras view. This is for the orange line follower. On a rotated image."""

        height = len(img)
        width = len(img[0])
        for i in range(5*height//8):#bottom black line
            img[i] = [[0,0,0] for j in range(width)]
        for i in range(7*height//8,height):#top black lines
            img[i] = [[0,0,0] for j in range(width)]
        return img
if __name__  == '__main__':
    rospy.init_node('line_location')
    ConeLocator()
    rospy.spin()
